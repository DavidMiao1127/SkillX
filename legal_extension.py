...
from SkillX.core.skill import Skill, SkillMetadata
from SkillX.core.skill_schema import UnifiedSkill, SkillExtractionResult

logger = logging.getLogger(__name__)

# --- 1. Custom Prompt for Legal Domain ---
...
def sanitize_category(category_str: str) -> str:
    """Ensure the category matches the expected L1 -> L2 structure, fallback if invalid."""
    valid_l1 = {
...
def dump_skills_to_markdown(skills_json: List[Dict], base_path: str = "./SkillBank"):
    import os
    import yaml
    ...
    for skill in skills_json:
        ...
        category = fm.get("category", "其他 -> 其他 -> 其他")
        
        # Enforce validation
        category = sanitize_category(category)
        fm["category"] = category
        
        # Build folder path based on taxonomy
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            # Replace escaped newlines with actual newlines in case the LLM outputs literal \n
            obj_bg = str(skill.get("objectives_and_background", "")).replace("\\n", "\n")
            f.write(obj_bg + "\n\n")
            
            f.write("## 2. Workflow Steps\n")
            wf_steps = str(skill.get("workflow_steps", "")).replace("\\n", "\n")
            f.write(wf_steps + "\n\n")
            ...
            f.write("## 4. Example\n")
            example = str(skill.get("example", "")).replace("\\n", "\n")
            f.write(example + "\n\n")
...
class LegalTextSkillExtractor(BaseSkillExtractor):
    ...
    async def extract(self, item: Dict, **kwargs) -> Optional[Dict]:
        ...
        # Phase 0: Pre-processing/Clustering using LLM
        # For simplicity, sending directly to main extraction in this MVP
        messages = [
...
# Note: Further integration into SkillX pipeline logic...
